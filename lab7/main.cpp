...
void Init_Light() {
    ...
    GLfloat light_position[] = { 10.0f, 0.0f, 7.0f, 1.0f };
    GLfloat light_spot_direction[] = {-10.0, 0.0, -7.0};
...
void DrawPrism(float x, float y, float z, float r, float h, float alpha) {
    ...
    for (i=0, zi=z; i<2; i++, zi+=h) {
        ...
        for (j=1, xi=x+r, yi=y; j<=10; j++, xi=nx, yi=ny){
            ...
            norm[i*30+j*3] = x-xi; norm[i*30+j*3+1] = y-yi; norm[i*30+j*3+2] = h/2.0*(2*i-1);
